...
def main() -> None:
    ...
    sections = [
        ...
        ("Dataset and Data Preparation", f"The downloaded Milan dataset contains {summary['file_count']} daily text files and {summary['row_count']:,} records across 10,000 squares. Each record is parsed as square ID, timestamp, and Internet traffic value, with variable-length activity fields handled by taking the final field. The raw files total approximately 20 GB and are streamed line by line, so peak memory is controlled by small numeric accumulators rather than a full raw table. Data are aggregated by square and 10-minute timestamp, then represented using lagged traffic, rolling statistics, and cyclical time features."),
